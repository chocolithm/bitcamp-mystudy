...
# 2024-10-02(수)
  ...
  - Dependency Injection : 의존객체를 직접 주입하지 않고 스프링으로 주입받는다.
  - Event Listener : Event가 발생했을 때 실행하는 Callback이나 Method (=Event Handler)


# 2024-10-04(금)
  ## Property Editor
  - IoC Container는 Primitive 변환기를 내장하고 있다.
  - 하지만 String을 그 외 타입으로 변환하려면 Property Editor를 설정해야 한다.
  - Property Editor는 Spring의 PropertyEditorSupport를 상속받아
    setAsText를 Overriding하면 된다.
  - 만든 Property Editor는 Spring의 CustomEditorConfigurer를 통해 등록한다.
  - JavaConfig에 @enableWebMVC를 등록하면 Date 타입 Editor를 자동으로 등록한다.

  ## AutoWired
  - @AutoWired를 통해 객체를 자동으로 주입할 수 있다.
  - AutoWired를 처리할 AutowiredAnnotationBeanPostProcessor가 등록되어야 한다.
  - BeanPostProcessor는 모든 Bean이 생성된 이후 객체를 찾아 주입한다.
  - 주입할 객체가 없거나 BeanPostProcessor가 없으면 예외가 발생한다.
  - Autowired보다는 생성자를 통해 객체를 주입받는 편이 장점이 많다.

  ## annotation-config
  - 자주 쓰는 Annotation의 실행 객체를 자동으로 등록한다.


# 2024-10-07(월)
  ## 취업특강
  - 참여 프로젝트 bullet 정리 (What, Why, How)

  ## DB Modeling
  - 정규화를 통해 DB 엔티티와 속성, 그 관계를 정의하고, 이를 통해 SQL 생성
  - ERD(Entity-Relationship Diagram)의 다양한 표기법 중 IE 표기법 사용
  - IE 표기법 : Information Engineering (Crow's Foot 표기법이라고도 함)
    1:1       A -|------|- B
    1:0또는1  A -|-----0|- B
    1:2이상   A -|-------< B
    1:1이상   A -|------|< B
  ...
  * DB Modeling 절차
  ...
  4. Candidate Key 선정
  5. Primary Key 선정
  6. 제1정규화 : 중복 컬럼, 중복 데이터 제거

  * 테이블 관계
  1. Non-identifying Relationship : FK != PK / 점선으로 표기
  2. Identifying Relationship : FK = PK / 실선으로 표기
...
# 2024-10-08(화)
  ...
  * DB Modeling 절차
  ...
  14. 물리 모델링
  15. 도메인 정의 및 적용
  16. auto_increment 적용
  17. 기본값 및 제약 조건 설정


# 2024-10-11(금)
  ## Spring WebMVC
  1. 요청이 들어오면 DispatcherServlet이 받는다. (Front Controller)
  - 클라이언트 요청을 접수
    - 요청을 처리할 PageController에게 전달
    - View 컴포넌트 실행
    - 요청 처리와 관련된 객체를 IoC Container에 등록하고 관리
  2. PageController에게 View 이름을 요청한다.
  - 요청 처리 담당
    - 클라이언트가 보낸 데이터를 가공
    - 적절한 Service 컴포넌트 실행
    - 결과 데이터를 View 컴포넌트가 쓸 수 있도록 가공
  3. ViewResolver에게 View를 요청한다.
  - View 이름으로 View 컴포넌트 조회
    - View 컴포넌트 반환
  4. View를 실행한다.
  - 클라이언트에게 보낼 콘텐트를 생성

  ## Page Controller 동작
  1. Controller가 요청을 받으면 Service를 호출한다.
    - 요청 데이터 가공
  2. Service는 DAO를 호춣한다.
    - 업무로직 수행
    - 트랜잭션 제어
    - 결과데이터 조정
  3. DAO는 DBMS에 질의하여 결과를 받는다.
    - DBMS에 데이터를 조회하거나 변경한다.
  4. Domain 객체를 생성하여 Service에 반환한다.
    - 클라이언트가 콘텐트를 만들 때 사용할 데이터
  5. Service는 Domain 객체를 조립하여 Controller에게 반환한다.
  6. 필요에 따라 데이터를 가공하고 Model에 Domain 객체를 보관한다.
  7. View 이름을 반환한다.

  ## View Component 동작
  1. DispatcherServlet이 Controller를 호출한다.
    - Controller를 호출할 때 RequestHandler 사용
  2. Controller는 사용할 데이터를 Model에 보관하고 View 이름을 반환한다.
  3. Model에 보관된 데이터를 ServletRequest로 옮긴다.
  4. DispatcherServlet이 ViewResolver에게 View를 요청하여 반환받는다.
  5. DispatcherServlet이 View를 실행한다.
  6. View는 ServletRequest에 담긴 데이터를 사용하여 콘텐트를 생성하고 반환한다.
    - HTML, Image, Excel, PDF, JSON, XML 등

  ## DispatcherServlet
  - Servlet Container(=Tomcat)가 web.xml을 읽어 DispatcherServlet을 생성한다.
  - DispatcherServlet은 XmlWebApplicationContext를 포함한다 (= IoC Container)
    - PageController 객체 보관
    - Web 관련 객체 보관
    - app-servlet.xml로 설정 (DispatcherServlet의 별명 + -servlet)
    - app-servlet.xml은 web.xml에 별도 설정하지 않으면 /WEB-INF/에서 찾는다.

  ## DispatcherServlet 등록 방법
  1. web.xml
  2. AppConfig
  3. Service-Provider
    - JVM이 App을 실행할 때 third-party가 제공한 프로그램이 자동 실행될 수 있도록 만든 규칙

  ## ContextLoaderListener
  - PageController가 사용할 의존 객체를 관리
  - ServletContextListener를 구현한 클래스로 app이 시작될 때 실행
  - ContextLoaderListener는 XmlWebApplicationContext를 생성 (= IoC Container)

  * DispatcherServlet의 XmlWebApplicationContext 객체가 없을 때,
    ContextLoaderListener의 XmlWebApplicationContext 객체를 찾는다.
  - 하지만 ContextLoaderListener의 IoC Container는 MVC 관련 애노테이션을 처리할 객체를
    등록하지 않아 별도로 활성화시켜야 한다. (<mvc:annotation-driven/>)

  * 실무에서는 Ioc Container를 다중으로 구성하기도 한다.


# 2024-10-14(월)
  ## WebApplicationInitializer
  - Spring의 SpringServletContainerInitializer가 WebApplicationInitializer 구현체를 실행
  - Servlet 인터페이스를 GenericServlet이 상속받아 구현했듯이,
    WebApplicationInitializer를 상속받은 추상클래스도 있다.

  ## AbstractContextLoaderInitializer
  - WebApplicationInitializer를 구현하는 추상클래스
  - registerContextLoaderListener()을 통해 ContextLoaderListener를 준비
  - 개발자는 DispatcherServlet만 준비하면 됨

  ## AbstractDispatcherServletInitializer
  - AbstractContextLoaderInitializer를 상속받는 추상클래스
  - registerDispatcherServlet()을 통해 DispatcherServlet을 준비
  - 개발자는 createServletApplicationContext()를 통해
    DispatcherServlet이 사용할 IoC Container만 준비하면 됨
  - getServletMappings()에서 매핑 설정
  - getServletFilters()에서 필터 설정
  - customizeRegistration()에서 부가 설정 (Multipart 등)

  ## AbstractAnnotationConfigDispatcherServletInitializer
  - AbstractDispatcherServletInitializer를 상속받는 추상클래스
  - getRootConfigClasses()에서 ContextLoaderListener의 IoC Container 설정
  - getServletConfigClasses()에서 DispatcherServlet의 IoC Container 설정


# 2024-10-16(수)
  ## PathVariable
  - 파라미터로 받지 않고 경로 자체에 변수를 받는 방법
  - Mapping에 중괄호로 설정

  ## MatrixVariable
  - url에 세미콜론으로 구분된 값을 받는 방법
